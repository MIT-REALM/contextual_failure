...
from contextual_failure.carla.carla import custom_posterior, get_costs, prepare_initial_model, fail_bo
import random
device = 'cuda'
...
def custom_posterior(Z):
    ...
    home_dir = f'/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}_{punchout_radius}_{delta_light}_{delta_dist}_{num_iter}_{num_init}'
...
def get_costs(object_list,ego_brake,lead_brake, sun_altitude):
    ...
    directory = f'/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}_{punchout_radius}_{delta_light}_{delta_dist}_{num_iter}_{num_init}/experiment_logs_{ego_brake:.2f}_{lead_brake:.2f}_{sun_altitude:.2f}'
...
def fail_bo(num_iter,bounds,X,Y,constraints,func,punchout_radius=0.1):
    id_ =0 
    New = False
    if New:
        num_actual = num_iter
    else:
        file = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}_{punchout_radius}_{delta_light}_{delta_dist}_{num_iter}_{num_init}.pkl", "rb")
    ...
    while id_ < num_actual:
        ...
        file = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}_{punchout_radius}_{delta_light}_{delta_dist}_{num_iter}_{num_init}.pkl", "wb")
...
def prepare_initial_model(num_init,seed):
    ...
    if New:
        for x in X:
            ...
            data = {'X':X, 'y_data':Y[1:,:]}
            file = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}/initial_data.pkl", "wb")
            pickle.dump(data,file )
            file.close()
    if not New:
        file = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal/seed_{seed}/initial_data.pkl", "rb")
        data_exists = pickle.load(file )
        file.close()
        ...
        for x in X[len_run:]:
            ...
            # model_gpr, mll = init_and_fit_gpr(torch.tensor(X_collect).double(), torch.tensor(np.array(y_data)).double().reshape(-1,1))
            data = {'X':X, 'y_data':Y}
            file = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_ECI_3_nominal_nominal/seed_{seed}/initial_data.pkl", "wb")
            pickle.dump(data,file )
            file.close()
    return X,Y
...
if __name__ == "__main__":
    ...
    if mode=='bo':
        ...
        file_init = open(f"/home/anjali/bayes_human/carla_sim/data_corl/experiment_3b/experiment_logs_random/seed_{3000}/initial_data.pkl", "rb")
        ...
        Z_list, y_data, gp_models = fail_bo(num_iter,bounds,X,Y,constraints,custom_posterior,punchout_radius)
